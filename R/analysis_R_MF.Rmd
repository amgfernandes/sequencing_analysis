...
```{r markers}
larva.markers %>% group_by(cluster) %>% top_n(n = 10,wt = avg_logFC)
```
...
larva.neurons <- FindAllMarkers(object = larva.neurons, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.8)
```
```{r}
larva.neurons %>% group_by(cluster) %>% top_n(n = 2, wt = avg_logFC)
```
