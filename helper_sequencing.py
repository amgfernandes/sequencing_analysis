...
class Sequencing():
    ...
    def remove_gene_list(self, dgc_mat, gene_list):
        '''Function receives a list of gene matrices and a
        list of genes and returns
        a gene matrix without the list gene rows.'''
        # The loaded 10x data is a dgc matrix.
        dgc_corr_res = []
        for dgc in dgc_mat:
            idx = self.return_indices_not_in_list(
                dgc.var_names, gene_list.gene.values)
    ...
    def keep_gene_list(self, dgc_mat, gene_list):
        ...
        dgc_corr_res = []
        for dgc in dgc_mat:
            idx = self.return_indices_inside_list(
                dgc.var_names, gene_list.gene.values)
            dgc_mat_corr = dgc[:, idx]
            dgc_corr_res.append(dgc_mat_corr)
        return (dgc_corr_res)
